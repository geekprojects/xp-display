...
public class CDUUfmc extends CDUSubcomponent {
    ...
    UfmcData ufmcData = null;
    XPlaneUDPSender udp_sender = null; 

    
    
    public CDUUfmc(ModelFactory model_factory, CDUGraphicsConfig cdu_gc, Component parent_component) {
        super(model_factory, cdu_gc, parent_component);
        
        try {
        	image = ImageIO.read(this.getClass().getResourceAsStream("/flightdeck/cdu/img/xfmc_2_800x480.png"));
		ap_img = ImageIO.read(this.getClass().getResourceAsStream("/flightdeck/cdu/img/xfmc_2_ap_litv_m.png"));
		athr_img = ImageIO.read(this.getClass().getResourceAsStream("/flightdeck/cdu/img/xfmc_2_athr_litv_m.png"));
		exec_img = ImageIO.read(this.getClass().getResourceAsStream("/flightdeck/cdu/img/xfmc_2_exec_litv_m.png"));
		lnav_img = ImageIO.read(this.getClass().getResourceAsStream("/flightdeck/cdu/img/xfmc_2_lnav_litv_m.png"));
		vnav_img = ImageIO.read(this.getClass().getResourceAsStream("/flightdeck/cdu/img/xfmc_2_vnav_litv_m.png"));
        ...
        ufmcData = UfmcData.getInstance();
        udp_sender = XPlaneUDPSender.get_instance();

        ufmcData.setLine(0, "Remote CDU for U-FMC");
        
        logger.finest("CDUUfmc instanciated");
    }
    
    
    public void paint(Graphics2D g2) {
    	if (cdu_gc.cdu_source == Avionics.CDU_SOURCE_UFMC) {
    ...
    private void drawFullPanel(Graphics2D g2) {
        ...
        if ( cdu_gc.powered ) {
            ...
            int stat = 0;
            try{
                stat = Integer.parseInt(ufmcData.getLine(14));
            } catch (Exception e) {}
    ...
    private void drawDisplayLines(Graphics2D g2, double dy) {
            ...
            for(int i=0; i < 14; i++){
                ...
                if (i == 13 ) {
                	...
                	// g2.setColor(i%2 == 1 ? cdu_gc.cdu_title_color : cdu_gc.cdu_data_color);	
                	g2.setColor(cdu_gc.cdu_data_color);
                }  
                
                String line_str=ufmcData.getLine(x);
                g2.drawString(line_str, xx, yy);
                /*
                List l = ufmcData.decodeLine(ufmcData.getLine(x));
                for(Object o : l){
                        Object[] pts = (Object[]) o;
    ...
    public void mousePressed(Graphics2D g2, MouseEvent e) {
    	if (cdu_gc.cdu_source == Avionics.CDU_SOURCE_UFMC) {
    		...
    		for(ClickRegion r : regions){
    			...
    			if(w > -1) {
    				udp_sender.sendDataPoint( XPlaneSimDataRepository.UFMC_KEY_PRESS, (float) w );
    				logger.info("UFMC CDU Mouse pressed : " + w);
    			}
    		}
    	}
    ...
    public void keyPressed(KeyEvent k) {
    	if (cdu_gc.cdu_source == Avionics.CDU_SOURCE_UFMC) {
    		...
    		if (w > -0.5f) udp_sender.sendDataPoint( XPlaneSimDataRepository.UFMC_KEY_PRESS, (float) w );
    		logger.info("UFMC CDU Key pressed : " + k.getKeyChar() + " " + w);
    	}    
    }
